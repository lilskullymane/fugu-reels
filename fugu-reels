...
local function save_project(num)
  if util.file_exists(_path.data .. "fugu/") == false then
    util.make_dir(_path.data .. "fugu/")
  end
  data[pattern].bpm = params:get("bpm")
  data.last_pattern = pattern
  data.mode = mode
  tab.save(data,_path.data .. "fugu/fugu-pat-"..num ..".data")
  params:write(_path.data .. "fugu/fugu-param-"..num ..".pset")
end

local function load_project(num)
  saved_data = tab.load(_path.data .. "fugu/fugu-pat-"..num ..".data")
  if saved_data ~= nil then
    data = saved_data
    pattern = data.last_pattern
    if data.mode ~= nil then mode = data.mode end
    params:read(_path.data .. "fugu/fugu-param-"..num ..".pset")
...
function init()
  ...
  for i=1,8 do
    ...
    if i < 5 then
      metro[i].on_step = function(stage) count(i,1) end
    elseif i >= 5 then
      metro[i].on_step = function(stage) count_oct(i - 4) end
    end
  end
  meta = beatclock.new()
  meta.on_step = metaseq
  meta.on_stop = function() data.meta_pos = 0 end
  metro_redraw = metro.init{event = function(stage) grid_redraw() redraw() end, time = 1 / 30}
  metro_redraw:start()
    ...
    if y == 8 then
      if x > 0 and x < 5 then
        if not alt then
          ...
          engine.noteOff(x)
          midi_out_device:note_off(last_note[x], 0, params:get(x.."midich"))
          states[x] = 0
